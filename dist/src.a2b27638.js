...
var dicecount = 3;
var sidescount = 6;
var die = _toConsumableArray(Array(6).keys()).map(function (s) {
  return ++s;
});
var dice = [];
for (var d = 0; d < dicecount; d++) {
  dice.push(_toConsumableArray(die));
}
...
var combinations = [];
var min = 0;
var max = Math.pow(sidescount, dicecount) - 1;
console.log('minmax', min, max);
for (var n = min; n <= max; n++) {
  var results = [];
  var sum = n;
  for (var _d = dicecount; _d > 0; _d--) {
    var value = Math.floor(sum / Math.pow(sidescount, _d - 1));
    //console.log ("d,s,v",d,sum,value);
    sum -= Math.pow(sidescount, _d - 1) * value;
